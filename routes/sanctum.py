...
async def list_tokens(user: Annotated[User, Depends(require_sanctum_auth)]) -> TokensListResponse:
    ...
    for token in tokens:
        token_responses.append(TokenResponse(
            ...
            expires_at=token.expires_at.isoformat() if hasattr(token.expires_at, 'isoformat') and token.expires_at else None,
            created_at=token.created_at.isoformat() if hasattr(token.created_at, 'isoformat') and token.created_at else None,
        ))
