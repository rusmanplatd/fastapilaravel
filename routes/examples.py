...
async def admin_only_route(current_user: User = Depends(can("view-dashboard"))):
    return {
...
async def super_admin_only_route(current_user: User = Depends(is_role("super-admin"))):
    return {
...
async def content_manager_route(
    current_user: User = Depends(has_any_permission(["create-posts", "edit-posts", "delete-posts"]))
):
    return {
...
async def admin_editor_route(
    current_user: User = Depends(has_all_roles(["admin", "editor"]))
):
    return {
...
async def flexible_access_route(current_user: User = Depends(get_current_user)):
...
async def permission_check_route(current_user: User = Depends(get_current_user)):
    checks = {
...
async def upload_file_route(current_user: User = Depends(can("upload-files"))):
