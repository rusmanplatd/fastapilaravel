...
async def get_entry(entry_uuid: str):
...
async def clear_entries(before: Optional[str] = Query(None)):
...
async def enable_watcher(watcher_name: str):
...
async def disable_watcher(watcher_name: str):
