...
class Factory(ABC):
    ...
    def definition(self) -> Dict[str, Any]:
        ...
        # Override this method to define your model's factory attributes
        # Example:
        # return {
        #     "name": fake.name(),
        #     "email": fake.email(),
    ...
    def make(self, attributes: Optional[Dict[str, Any]] = None) -> Any:
    ...
    def create(self, attributes: Optional[Dict[str, Any]] = None) -> Any:
    ...
    def with_relations(self, **relations: Any) -> Factory:
        """Set up relationships to be created with the model"""
        factory = self._clone()
        factory.with_relationships.update(relations)
        return factory
    
    def has(self, relation_name: str, factory_or_callback: Union[Factory, Callable[..., Any]], count: int = 1) -> Factory:
        """Create related models using has* relationship pattern"""
        factory = self._clone()
        
        if isinstance(factory_or_callback, Factory):
            related_factory: Any = factory_or_callback.times(count)
        else:
    ...
    def for_relation(self, parent_model: Any, relation_key: Optional[str] = None) -> Factory:
