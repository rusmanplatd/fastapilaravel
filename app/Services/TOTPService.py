...
class TOTPService(BaseService):
    """TOTP service with improved security, QR codes, and validation"""
    ...
    def generate_secret(self, user: Optional[User] = None) -> str:
    ...
    def generate_qr_code_with_logo(
        ...
        if logo_data:
            try:
                ...
                qr_width, qr_height = qr_img.size  # type: ignore[attr-defined]
                ...
                qr_img.paste(logo, pos, logo)  # type: ignore[attr-defined]
    ...
    def setup_totp(
        ...
        """Setup TOTP for user with enhanced security and options"""
        try:
            # Check rate limiting
            ...
            qr_code = self.generate_qr_code(provisioning_uri, qr_style)
            ...
            return True, "TOTP setup initiated", {
                "qr_code": qr_code,
                "qr_code_svg": self.generate_qr_code(provisioning_uri, "svg"),
    ...
    def verify_and_enable_totp(
        ...
        """Verify TOTP token and enable TOTP for user with enhanced security logging"""
        try:
            # Check rate limiting
            rate_check = self.rate_limit_service.check_rate_limit(
                user, MFAAttemptType.TOTP, ip_address=ip_address, user_agent=user_agent
            ...
            verified, drift = self.verify_token(
    ...
    def verify_totp_for_login(
        ...
        """Verify TOTP token during login with detailed logging"""
        try:
            # Check rate limiting
            rate_check = self.rate_limit_service.check_rate_limit(
    ...
    def disable_totp(self, user: User, token: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            verified, _ = self.verify_token(user.mfa_settings.totp_secret, token) if user.mfa_settings.totp_secret else (False, 0)
            if not user.mfa_settings.totp_secret or not verified:
    ...
    def regenerate_backup_codes(self, user: User, totp_token: str) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            # Verify TOTP token
            verified, _ = self.verify_token(user.mfa_settings.totp_secret, totp_token) if user.mfa_settings.totp_secret else (False, 0)
            if not user.mfa_settings.totp_secret or not verified:
                return False, "Invalid TOTP token", []
    ...
    def _verify_backup_code(self, user: User, code: str) -> bool:
        """Verify and consume a backup code with usage tracking"""
    ...
    def get_totp_status(self, user: User) -> Dict[str, Any]:
        """Get TOTP status and statistics"""
        if not user.mfa_settings:
            return {
                "enabled": False,
                "setup_required": True,
