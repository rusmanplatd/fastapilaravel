...
class TOTPService(BaseService):
    ...
    def generate_qr_code(self, provisioning_uri: str) -> str:
        """Generate QR code image as base64 string (legacy method)"""
        return self.generate_styled_qr_code(provisioning_uri, "default")
    ...
    def verify_token(self, secret: str, token: str, window: int = 1) -> bool:
        """Verify TOTP token (legacy method)"""
        verified, _ = self.verify_token_with_drift_detection(secret, token, window)
        return verified
    ...
    def setup_totp(self, user: User, issuer: str = "FastAPI Laravel") -> Tuple[bool, str, Dict[str, Any]]:
        """Setup TOTP for user and return QR code data (legacy method)"""
        return self.setup_totp_enhanced(user, issuer)
    ...
    def verify_and_enable_totp(self, user: User, token: str) -> Tuple[bool, str]:
        """Verify TOTP token and enable TOTP for user (legacy method)"""
        success, message, _ = self.verify_and_enable_totp_enhanced(user, token)
    ...
    def verify_totp_for_login(self, user: User, token: str) -> Tuple[bool, str]:
        """Verify TOTP token during login (legacy method)"""
        success, message, _ = self.verify_totp_for_login_enhanced(user, token)
        return success, message
    ...
    def regenerate_backup_codes(self, user: User, totp_token: str) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            # Verify TOTP token
            if not user.mfa_settings.totp_secret or not self.verify_token(user.mfa_settings.totp_secret, totp_token):
                return False, "Invalid TOTP token", []
    ...
    def _verify_backup_code(self, user: User, code: str) -> bool:
        """Verify and consume a backup code (legacy method)"""
        return self._verify_backup_code_enhanced(user, code)
