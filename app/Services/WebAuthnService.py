...
class WebAuthnService(BaseService):
        ...
        try:
            ...
            for credential in user.webauthn_credentials:
                existing_credentials.append({
                    "id": credential.credential_id.encode('latin-1') if hasattr(credential.credential_id, 'encode') else credential.credential_id,
                    "type": "public-key"
                })
    ...
    def _get_device_metadata(self, credential: WebAuthnCredential) -> Dict[str, Any]:
    ...
    def _calculate_credential_risk_score(self, credential: WebAuthnCredential) -> int:
