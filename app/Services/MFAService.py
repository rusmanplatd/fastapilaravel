...
class MFAService(BaseService):
        ...
        return {
            ...
            "methods": {
                ...
                "webauthn": {
                    ...
                    "credentials_count": len(getattr(user, 'webauthn_credentials', []))
                },
                "sms": {
    ...
    def get_mfa_session(self, session_token: str) -> Optional[MFASession]:  # type: ignore[no-any-unimported]
    ...
    def verify_mfa_with_totp(self, session_token: str, totp_code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            user = session.user
            success, message, _ = self.totp_service.verify_totp_for_login(user, totp_code)
    ...
    def verify_mfa_with_webauthn(self, session_token: str, credential: Dict[str, Any], challenge: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            user = session.user
            success, message, _ = self.webauthn_service.verify_authentication(user, credential, challenge)
    ...
    def generate_sms_code(self, user: User) -> Tuple[bool, str]:
        ...
        try:
            if not user.mfa_settings or not user.mfa_settings.sms_enabled:                return False, "SMS MFA is not enabled for this user"
            
            if not user.mfa_settings.sms_phone_number:                return False, "No phone number configured for SMS MFA"
    ...
    def disable_all_mfa(self, user: User, verification_method: str, verification_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if verification_method == "totp" and "token" in verification_data:
                success, _, _ = self.totp_service.verify_totp_for_login(user, verification_data["token"])
                verified = success
            elif verification_method == "webauthn" and "credential" in verification_data and "challenge" in verification_data:
                success, _, _ = self.webauthn_service.verify_authentication(
    ...
    def get_available_mfa_methods(self, user: User) -> List[str]:
        ...
        if not user.mfa_settings:            return []
        
        methods = []
        if user.mfa_settings.totp_enabled:            methods.append("totp")
        if user.mfa_settings.webauthn_enabled:            methods.append("webauthn")
        if user.mfa_settings.sms_enabled:            methods.append("sms")
