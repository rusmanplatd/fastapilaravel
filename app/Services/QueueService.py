...
if TYPE_CHECKING:
    ...
    from app.Models.FailedJob import FailedJob
...
class QueueService(BaseService):
    ...
    def push(self, job: ShouldQueue, queue: Optional[str] = None) -> str:
        ...
        try:
            ...
            job_model = JobModel(
                ...
                job_method="handle",
                connection=job.options.connection,
                available_at=datetime.utcnow(),
                priority=job.options.priority,
                attempts=job.attempts,
                timeout=job.options.timeout,
                retry_after=job.options.retry_delay,
                max_attempts=job.options.max_attempts,
                backoff=job.options.retry_delay,
                tags=job.get_tags()
            )
    ...
    def size(self, queue: str = "default") -> int:
        ...
        try:
            ...
            count = db.query(JobModel).filter(
