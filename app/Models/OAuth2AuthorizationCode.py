...
class OAuth2AuthorizationCode(BaseModel):
    """OAuth2 Authorization Code model for Laravel 12 enhanced authentication."""
    ...
    oidc_params: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON storage for OIDC parameters
    
    # Associations
    user_id: Mapped[ULID] = mapped_column(String, ForeignKey("users.id"), nullable=False)  # type: ignore[arg-type]
    ...
    def get_oidc_params(self) -> Dict[str, Any]:
        """Get OpenID Connect parameters from stored JSON."""
        if not self.oidc_params:
            return {}
        import json
        try:
            return json.loads(self.oidc_params)
        except (json.JSONDecodeError, TypeError):
            return {}
    
    def set_oidc_params(self, params: Dict[str, Any]) -> None:
        """Set OpenID Connect parameters as JSON."""
        import json
        self.oidc_params = json.dumps(params) if params else None
    
    def get_oidc_param(self, key: str, default: Any = None) -> Any:
        """Get specific OpenID Connect parameter."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "nonce": self.nonce,
            "oidc_params": self.get_oidc_params(),
        }