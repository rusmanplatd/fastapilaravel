...
from sqlalchemy import Table, Column, ForeignKey, String, text, and_, or_, func
...
class BelongsToManyRelation(Generic[T]):
    ...
    def attach(self, session: Session, model_ids: List[Any], pivot_data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def detach(self, session: Session, model_ids: Optional[List[Any]] = None) -> None:
    ...
    def sync(self, session: Session, model_ids: List[Any], pivot_data: Optional[Dict[str, Any]] = None) -> None:
        """Sync the relationship - detach all existing and attach new"""
        self.detach(session)
        if model_ids:
            self.attach(session, model_ids, pivot_data)
    
    def toggle(self, session: Session, model_ids: List[Any], pivot_data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def with_pivot(self, session: Session, *columns: str) -> List[T]:
