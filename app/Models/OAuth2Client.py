...
class OAuth2Client(BaseModel):
    """OAuth2 Client model for Laravel 12 enhanced authentication."""
    ...
    allowed_scopes: Mapped[str] = mapped_column(nullable=False, default="openid,profile,email")
    grant_types: Mapped[str] = mapped_column(nullable=False, default="authorization_code,refresh_token")
    response_types: Mapped[str] = mapped_column(nullable=False, default="code,id_token,token")
    
    # Google IDP-style fields
    logo_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    client_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    policy_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    tos_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    jwks_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    sector_identifier_uri: Mapped[Optional[str]] = mapped_column(nullable=True)
    subject_type: Mapped[str] = mapped_column(nullable=False, default="public")
    id_token_signed_response_alg: Mapped[str] = mapped_column(nullable=False, default="RS256")
    token_endpoint_auth_method: Mapped[str] = mapped_column(nullable=False, default="client_secret_basic")
    ...
    require_auth_time: Mapped[bool] = mapped_column(default=False, nullable=False)
    default_max_age: Mapped[Optional[int]] = mapped_column(nullable=True)
    require_pushed_authorization_requests: Mapped[bool] = mapped_column(default=False, nullable=False)
    ...
    def supports_openid_connect(self) -> bool:
        """Check if client supports OpenID Connect."""
        return "openid" in self.get_allowed_scopes()
