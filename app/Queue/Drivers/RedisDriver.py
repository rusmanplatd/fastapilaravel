...
try:
    ...
    RedisClass = None  # type: ignore[misc,assignment]
...
class RedisQueueDriver:
    ...
    def pop(self, queue: str = "default", timeout: int = 10) -> Optional[Dict[str, Any]]:
        ...
        job_data: Dict[str, Any] = json.loads(job_data_json)
    ...
    def get_queue_size(self, queue: str = "default") -> int:
        ...
        return int(self.redis.zcard(f"{self.key_prefix}{queue}"))
    
    def get_delayed_count(self) -> int:
        """Get number of delayed jobs."""
        return int(self.redis.zcard(f"{self.key_prefix}delayed"))
    
    def get_reserved_count(self) -> int:
        """Get number of reserved jobs."""
        return int(self.redis.scard(f"{self.key_prefix}reserved"))
    
    def get_failed_count(self) -> int:
        """Get number of failed jobs."""
        return int(self.redis.hlen(f"{self.key_prefix}failed"))
    ...
    def clear_queue(self, queue: str = "default") -> int:
        ...
        # Clear queue
        count = int(self.redis.zcard(f"{self.key_prefix}{queue}"))
    ...
    def clear_failed_jobs(self) -> int:
        ...
        count = int(self.redis.hlen(f"{self.key_prefix}failed"))
