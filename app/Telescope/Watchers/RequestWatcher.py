...
class RequestWatcher(TelescopeWatcher):
    ...
    def record_request(
        ...
        if self.should_ignore(str(request.url)):  # type: ignore[attr-defined]
            return
        
        # Extract request data
        content = {
            'uri': str(request.url),  # type: ignore[attr-defined]
            ...
            'response_status': response.status_code,  # type: ignore[attr-defined]
            'response': self._get_response_content(response),
            'duration': duration,
            'memory': memory_peak or 0,
            'hostname': request.client.host if request.client else 'unknown',  # type: ignore[attr-defined]
            'user': self._get_authenticated_user(request),
        }
        
        # Create tags for filtering
        tags = [
            f"method:{request.method.lower()}",  # type: ignore[attr-defined]
            f"status:{response.status_code}",  # type: ignore[attr-defined]
