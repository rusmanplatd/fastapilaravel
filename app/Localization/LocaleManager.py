...
class LocaleDetector:
    ...
    def detect_from_subdomain(self, request: Any, supported_locales: List[str]) -> Optional[str]:
        ...
        if not hasattr(request, 'url') or not hasattr(request.url, 'hostname'):  # type: ignore[attr-defined]
            return None
        
        hostname = request.url.hostname  # type: ignore[attr-defined]
        if not hostname:
            return None
    ...
    def detect_from_path_prefix(self, request: Any, supported_locales: List[str]) -> Optional[str]:
        ...
        if not hasattr(request, 'url') or not hasattr(request.url, 'path'):  # type: ignore[attr-defined]
            return None
        
        path = request.url.path.strip('/')  # type: ignore[attr-defined]
        if not path:
            return None
