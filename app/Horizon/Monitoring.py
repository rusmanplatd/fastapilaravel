...
class JobStatus:
    ...
    def __post_init__(self) -> None:
        # payload is now initialized with default_factory
        pass
...
class JobMonitor:
    ...
    async def _update_job_metrics(self, event: str, queue: str, processing_time: Optional[float] = None) -> None:
...
class QueueMonitor:
    ...
    async def _store_queue_metrics(self, status: QueueStatus) -> None:
        ...
        if metrics_data.get('last_processed_at'):
            metrics_data['last_processed_at'] = status.last_processed_at.isoformat() if status.last_processed_at else None
