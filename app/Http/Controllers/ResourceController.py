...
class ResourceController(BaseController, Generic[T, ResourceType], ABC):
    ...
    def build_index_query(self, request: Request) -> Any:
        """Build the base query for index method"""
        # This would return a SQLAlchemy query
        # Implementation depends on your database setup
        if not self.model_class:
            raise ValueError("model_class must be defined")
        # TODO: Implement with proper database session
    ...
    def apply_sorting(self, query: Any, sort_by: str, sort_order: str) -> Any:
        """Apply sorting to query"""
        # Override in subclasses to implement sorting
        return query
    
    async def count_query(self, query: Any) -> int:
        """Count total records for pagination"""
        # Implementation depends on your database setup
        return 0
    
    async def paginate_query(self, query: Any, offset: int, limit: int) -> List[T]:
    ...
    async def find_or_fail(self, id: str) -> T:
        ...
        if not self.model_class:
            raise ValueError("model_class must be defined")
        # Using a placeholder since find() method doesn't exist
        # In a real implementation, you'd query your database here
        # For now, always raise not found to indicate this needs implementation
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"{self.model_class.__name__} not found - find_or_fail method needs implementation"
        )
    ...
    async def create_item(self, data: Dict[str, Any], request: Request) -> T:
        ...
        # await item.save()  # Placeholder - implement actual save logic
        return item
    ...
    async def delete_item(self, item: T, request: Request) -> None:
        """Delete item"""
        # await item.delete()  # Placeholder - implement actual delete logic
        pass
