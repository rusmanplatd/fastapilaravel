...
class SocialAuthController(BaseController):
    ...
    async def handle_provider_callback(
        ...
        try:
            ...
            token_data = self.auth_service._generate_tokens(user)
    ...
    async def _find_or_create_user(self, social_user: Any, provider: str) -> User:
        ...
        if not user and SOCIALITE_SETTINGS.get('auto_create_users'):
            ...
            if default_role:
                await self._assign_default_role(user, str(default_role), db)
    ...
    async def _assign_default_role(self, user: User, role_name: str, db: Any) -> None:
        ...
        try:
            ...
            if role:
                # Use proper role assignment method
                if hasattr(user, 'assign_role'):
                    user.assign_role(role)  # Pass Role object, not string
                # Alternative: direct database insertion if needed
                db.commit()
        except Exception:
