...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        if request.method not in self.cacheable_methods:  # type: ignore[attr-defined]
    ...
    def _get_cache_headers(self, request: Request) -> Dict[str, str]:
        ...
        for header in relevant_headers:
            if header in request.headers:  # type: ignore[attr-defined]
                cache_headers[header] = request.headers[header]  # type: ignore[attr-defined]
    ...
    def _should_cache_response(self, response: Response) -> bool:
        ...
        if response.status_code < 200 or response.status_code >= 300:  # type: ignore[attr-defined]
            return False
        
        # Check cache control headers
        cache_control = response.headers  # type: ignore[attr-defined].get('cache-control', '')
...
class ResponseCacheMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        if response.status_code:  # type: ignore[attr-defined] == 200
    ...
    def _generate_rule_cache_key(self, request: Request, rule: Dict[str, Any]) -> str:
        ...
        key_parts = [request.url.path, request.method]  # type: ignore[attr-defined]
...
class CacheTagMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        request.state.cache_tags  # type: ignore[attr-defined] = self.default_tags.copy()
        
        response = await call_next(request)
        
        # Add cache tags header for debugging
        if hasattr(request.state, 'cache_tags') and request.state.cache_tags:  # type: ignore[attr-defined]
            response.headers  # type: ignore[attr-defined]['X-Cache-Tags'] = ','.join(request.state.cache_tags  # type: ignore[attr-defined])
