...
class MFAMiddleware:
    """Enhanced MFA middleware with policy enforcement and security features"""
    ...
    async def __call__(
        ...
        """Enhanced MFA middleware with comprehensive security checks"""
        try:
            ...
            try:
                ...
                if mfa_session_token:
                    ...
                    if session_valid:
                        ...
                        request.state.mfa_verified  # type: ignore[attr-defined] = True
                        request.state.mfa_method  # type: ignore[attr-defined] = session_info.get("method_used") if session_info else None
    ...
    def _gather_request_context(self, request: Request, user: User) -> Dict[str, Any]:
        ...
        context["user_agent"] = request.headers.get("user-agent", "")  # type: ignore[attr-defined] if hasattr(request, 'headers') else ""
    ...
    def _extract_mfa_session_token(self, request: Request) -> Optional[str]:
        ...
        legacy_token = request.headers.get("X-MFA-Session-Token")
        if legacy_token:
            return legacy_token
...
def require_mfa_legacy(user: User, db: Session) -> Optional[JSONResponse]:
    """
    Legacy helper function to check MFA requirements for specific endpoints
...
def verify_mfa_session(session_token: str, user: User, db: Session) -> bool:
    """
    Legacy helper function to verify MFA session token
    """
    try:
        mfa_service = MFAService(db)
        mfa_session = mfa_service.get_mfa_session(session_token)
