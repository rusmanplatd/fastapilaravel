...
class MiddlewareManager:
    ...
    def when(self, condition: Union[bool, Callable[[], bool]], *middleware: str) -> 'MiddlewareManager':
        ...
        for name in middleware:
            if name in self.registrations:
                ...
                self.registrations[name].config.conditions.append(condition_func)
        return self
    ...
    def unless(self, condition: Union[bool, Callable[[], bool]], *middleware: str) -> 'MiddlewareManager':
        ...
        inverted_condition = (lambda: not condition()) if callable(condition) else not condition
    ...
    def aliases(self, alias_dict: Dict[str, str]) -> 'MiddlewareManager':
    ...
    def group(self, name: str, middleware_list: List[str]) -> None:
        """Laravel-style middleware group registration."""
        self.middleware_groups[name] = middleware_list
        self.logger.info(f"Registered middleware group '{name}': {middleware_list}")
    
    def alias(self, alias: str, middleware_name: str) -> None:
        """Register a middleware alias."""
        self.middleware_aliases[alias] = middleware_name
        self.logger.info(f"Registered middleware alias '{alias}' -> '{middleware_name}'")
    
    def when(self, condition: Callable[[], bool], middleware_names: List[str]) -> None:
        """Register conditional middleware."""
        self.conditional_middleware.append({
            "condition": condition,
            "middleware": middleware_names
        })
        self.logger.info(f"Registered conditional middleware: {middleware_names}")
    
    def unless(self, condition: Callable[[], bool], middleware_names: List[str]) -> None:
        """Register middleware that runs unless condition is true."""
        def inverted_condition() -> bool:
            return not condition()
        
        self.when(inverted_condition, middleware_names)
    ...
    def disable(self, middleware_name: str) -> None:
        ...
        if middleware_name in self.registered_middleware:
            self.registered_middleware[middleware_name]["enabled"] = False
            self.logger.info(f"Disabled middleware: {middleware_name}")
    
    def enable(self, middleware_name: str) -> None:
        """Enable a middleware."""
        if middleware_name in self.registered_middleware:
            self.registered_middleware[middleware_name]["enabled"] = True
            self.logger.info(f"Enabled middleware: {middleware_name}")
    
    def replace(self, old_middleware: str, new_middleware: str) -> None:
        """Replace a middleware with another."""
        if old_middleware in self.registered_middleware and new_middleware in self.registered_middleware:
            # Copy settings from old to new
            old_settings = self.registered_middleware[old_middleware]
            self.registered_middleware[new_middleware].update({
                "priority": old_settings["priority"],
                "enabled": old_settings["enabled"]
            })
            
            # Disable old middleware
            self.registered_middleware[old_middleware]["enabled"] = False
            
            self.logger.info(f"Replaced middleware '{old_middleware}' with '{new_middleware}'")
    ...
    def get_group_middleware(self, group_name: str) -> List[str]:
        ...
        return self.middleware_groups.get(group_name, [])
    
    def resolve_middleware_name(self, name: str) -> str:
        """Resolve middleware name through aliases."""
        return self.middleware_aliases.get(name, name)
    ...
    def _resolve_all_middleware(self) -> List[str]:
        ...
        all_middleware.update(self.middleware_order)
        ...
        for middleware_name in all_middleware:
            ...
            if resolved_name in self.middleware_groups:
    ...
    def get_middleware_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "middleware_aliases": self.middleware_aliases,
...
def setup_production_middleware(app: FastAPI) -> None:
    ...
    manager.registered_middleware["trusted_host"]["enabled"] = True
    manager.registered_middleware["trusted_host"]["config"]["allowed_hosts"] = [
        "yourdomain.com", "*.yourdomain.com", "api.yourdomain.com"
    ]
    
    # Configure performance monitoring for production
    manager.registered_middleware["performance"]["config"].update({
...
def setup_development_middleware(app: FastAPI) -> None:
    ...
    manager.registered_middleware["trusted_host"]["enabled"] = False
