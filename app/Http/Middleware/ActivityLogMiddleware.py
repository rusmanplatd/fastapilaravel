...
class ActivityLogMiddleware(BaseHTTPMiddleware):
    ...
    def _should_log_request(self, request: Request, response: Response) -> bool:
        ...
        if any(request.url.path.startswith(path) for path in self.exclude_paths):  # type: ignore[attr-defined]
            return False
        
        # Check if we only log successful requests
        if self.log_successful_only and not (200 <= response.status_code < 300):  # type: ignore[attr-defined]
            return False
    ...
    async def _log_http_request(self, request: Request, response: Response, start_time: float) -> None:
        ...
        try:
            ...
            properties = {
                ...
                "user_agent": request.headers.get("user-agent"),  # type: ignore[attr-defined]
                "ip_address": self._get_client_ip(request),
                "referer": request.headers.get("referer")  # type: ignore[attr-defined]
            }
            ...
            if request.method != "GET":  # type: ignore[attr-defined]
                ...
                properties["content_type"] = request.headers.get("content-type")  # type: ignore[attr-defined]
                properties["content_length"] = request.headers.get("content-length")  # type: ignore[attr-defined]
            
            # Determine description based on status code
            if 200 <= response.status_code < 300:  # type: ignore[attr-defined]
                description = f"Successful {request.method} request to {request.url.path}"  # type: ignore[attr-defined]
            elif 400 <= response.status_code < 500:  # type: ignore[attr-defined]
                description = f"Client error {request.method} request to {request.url.path}"  # type: ignore[attr-defined]
            elif 500 <= response.status_code < 600:  # type: ignore[attr-defined]
                description = f"Server error {request.method} request to {request.url.path}"  # type: ignore[attr-defined]
            else:
                description = f"{request.method} request to {request.url.path}"  # type: ignore[attr-defined]
    ...
    async def _log_failed_request(self, request: Request, exception: Exception, start_time: float) -> None:
        ...
        try:
            ...
            description = f"Failed {request.method} request to {request.url.path}: {exception.__class__.__name__}"  # type: ignore[attr-defined]
    ...
    def _get_client_ip(self, request: Request) -> Optional[str]:
        ...
        if hasattr(request, "client") and request.client:  # type: ignore[attr-defined]
