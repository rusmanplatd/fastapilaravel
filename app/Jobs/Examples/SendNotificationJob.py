...
class SendNotificationJob(Job):
    ...
    def handle(self) -> None:
        ...
        try:
            ...
            elif self.notification_type == "system_update":
                from app.Notifications.Examples.SystemUpdateNotification import SystemUpdateNotification
                update_type = self.notification_data.get("update_type")
                update_details = self.notification_data.get("update_details")
                if not update_type or not update_details:
                    raise ValueError("update_type and update_details are required for system_update notifications")
                notification = SystemUpdateNotification(
