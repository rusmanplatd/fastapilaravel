...
def custom_filters_example(db: Session, request: Request) -> List[User]:
    ...
    def filter_users_with_posts(query: Query[User], value: bool, property_name: str) -> Query[User]:
        # Example filter - User.posts relationship would need to be defined in the User model
        if value:
            # return query.join(User.posts).distinct()
            return query.distinct()  # Placeholder implementation
        else:
    ...
    def filter_by_permission(query: Query[User], value: str, property_name: str) -> Query[User]:
        # Placeholder implementation - would need proper model reference
        return query  # .join(User.direct_permissions).filter(Permission.name == value)
...
def fastapi_dependency_example() -> None:
    ...
    user_list_dependency = create_list_endpoint_dependency(
        model_class=User,
        get_db=lambda: next(get_db_session()),  # Convert generator to callable
        allowed_filters=['name', 'email', 'is_active'],
        allowed_sorts=['name', 'created_at'],
