...
class QueryBuilderRequest:
    ...
    def __init__(self, request: Request) -> None:
        ...
        self._parsed_filters: Optional[Dict[str, Union[str, int, bool, List[str], Dict[str, str]]]] = None
    ...
    def filters(self) -> Dict[str, Union[str, int, bool, List[str], Dict[str, str]]]:
        """Get parsed filters"""
    ...
    def _parse_filters(self) -> Dict[str, Union[str, int, bool, List[str], Dict[str, str]]]:
        """Parse filter parameters"""
        filters: Dict[str, Union[str, int, bool, List[str], Dict[str, str]]] = {}
        ...
        for param_name, param_value in self._query_params.items():
            ...
            if filter_match:
                filter_name: str = filter_match.group(1)
    ...
    def _parse_filter_value(self, value: str) -> Union[str, List[str], Dict[str, str]]:
        ...
        if dynamic_operator_match:
            operator: str = dynamic_operator_match.group(1)
            val: str = dynamic_operator_match.group(2)
    ...
    def _parse_fields(self) -> Dict[str, List[str]]:
        ...
        for param_name, param_value in self._query_params.items():
            ...
            if fields_match:
                table_name: str = fields_match.group(1)
    ...
    def get_page(self, default: int = 1) -> int:
        ...
        page_str = self.request.query_params.get('page', str(default))
        try:
            return max(1, int(page_str))
        except (ValueError, TypeError):
            return default
    
    def get_per_page(self, default: int = 15) -> int:
        """Get per_page number from query parameters"""
        per_page_str = self.request.query_params.get('per_page', str(default))
        try:
            return max(1, min(100, int(per_page_str)))  # Cap at 100
        except (ValueError, TypeError):
            return default