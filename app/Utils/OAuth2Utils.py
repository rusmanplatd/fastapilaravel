...
from app.Types.JsonTypes import JsonObject, JWTHeader, JWTPayload
...
class OAuth2TokenUtils:
    ...
    def is_valid_jwt_format(token: str) -> bool:
        ...
        try:
            ...
            header_data = cast(JWTHeader, json.loads(base64.urlsafe_b64decode(parts[0] + '===')))
            payload_data = cast(JWTPayload, json.loads(base64.urlsafe_b64decode(parts[1] + '===')))
            
            # Basic JWT structure checks
            return (
                isinstance(header_data, dict) and
                isinstance(payload_data, dict) and
                'alg' in header_data
            )
    ...
    @staticmethod
    def extract_jwt_payload(token: str) -> Optional[JsonObject]:
        """
        ...
        try:
            ...
            decoded_payload = cast(JWTPayload, json.loads(payload))
            return cast(JsonObject, decoded_payload)
        except (json.JSONDecodeError, ValueError):
            return None
...
class OAuth2CryptoUtils:
    ...
    def create_jwt_token(
        payload: Dict[str, Union[str, int, bool, None, datetime]],
        private_key: str,
        algorithm: str = 'RS256',
    ...
    def verify_jwt_token(
        ...
        """
        try:
            decoded_token = cast(JWTPayload, jwt.decode(token, public_key, algorithms=[algorithm]))
            return cast(Dict[str, Union[str, int, bool, None]], decoded_token)
        except JWTError:
            return None